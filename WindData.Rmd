...
# Example: get the clause (optional, but often useful for debugging)
...
# -------------------------
journal_data$wind_clause      <- extract_wind_clause(journal_data$journal_entry)
journal_data$wind_dir_token   <- extract_first_wind_direction(journal_data$journal_entry)

journal_data$wind_dir_token_2   <- extract_second_wind_direction(journal_data$journal_entry)
...
normalize_direction <- function(x) {
  ...
  # handle simple abbreviations like n, nw, sse
  ...
  # look up word forms
  if (x_clean %in% names(compass_map))
    return(compass_map[[x_clean]])

  # otherwise return NA
  return(NA_character_)
}

journal_data$wind_dir_clean <- vapply(
  journal_data$wind_dir_token,
  normalize_direction,
  character(1)
)

journal_data$wind_dir_clean_2 <- vapply(
  journal_data$wind_dir_token_2,
  normalize_direction,
  character(1)
)


journal_data |> filter(!is.na(wind_mentioned)) |> select(journal_entry, wind_clause, wind_dir_token, wind_dir_token_2, wind_mentioned, wind_dir_clean, wind_dir_clean_2)
```

```{r original wind direction patterns, eval = FALSE}
...
visdat::vis_miss_summary(journal_data_geocoded)
```
