...
```{r}
ggplot(beaufort_grouped_math,
       aes(x = wind_dir_math_real, y = factor(beaufort), fill = frequency)) +
  geom_tile() +
...
# jpeg("mosaic.jpg", width = 7, height = 8, units = "in", res = 300)

mosaicplot(
  freq_tbl,
  color = TRUE,
  las = 2,
  main = "Beaufort vs Wind Direction"
)
...
journal_wind_df$dir_bin <- cut(
  journal_wind_df$wind_dir_math_real,
  breaks = seq(0, 360, by = 45),
  include.lowest = TRUE
)

freq_tbl_bin <- with(journal_wind_df,
                     table(beaufort, dir_bin))

barplot(freq_tbl_bin, las = 2)
...
hist(journal_wind_df$wind_dir_math_real,
     breaks = seq(0, 360, by = 15),
     freq = TRUE,
     main = "Wind Direction Frequency",
     xlab = "Degrees")

with(subset(journal_wind_df, beaufort >= 4),
     hist(wind_dir_math_real, breaks = seq(0, 360, by = 15),
          add = TRUE, col = rgb(1, 0, 0, 0.3)))
...
ggplot(
  journal_wind_df |> 
    filter(!is.na(wind_direction), !is.na(beaufort)),
  aes(x = wind_direction, fill = factor(beaufort))
) +
...
journal_wind_df |> 
  ...
  ggplot(aes(x = dir_bin, fill = factor(beaufort))) +
...
```{r}
journal_wind_df |>
  filter(wind_direction == "No wind", !is.na(beaufort)) |>
  print(n = Inf)
...
ggplot(missing_speed, aes(wind_meteor)) +
...
missing_speed$count <- 1

ggplot(missing_speed, aes(x = wind_direction, y = count)) +
...
journals_duplicates <- journals [
  duplicated(journals$date_mdy) | duplicated(journals$date_mdy, fromLast = TRUE),
]

journals_duplicates <- journals_duplicates %>%
  arrange(date_mdy)
