...
upset_df <- journal_wind_df %>%
  ...
  select(
    WindMentioned, HasDirection, StrongWind, Winter,
    beaufort, wind_dir_math_real, month_numeric, wind_speed_knots, year_numeric
  )
...
upset_df$wind_dir_math_real <- as.numeric(upset_df$wind_dir_math_real)
...
```{r}
sets <- c("WindMentioned", "HasDirection", "StrongWind", "Winter")
...
jpeg("upset3.jpg", width = 10, height = 20, units = "in", res = 300)
...
upset(
  ...
  attribute.plots = list(
    gridrows = 60,
    plots = list(
      list(plot = scatter_plot, x = "wind_dir_math_real", y = "beaufort"),
      list(plot = scatter_plot, x = "month_numeric", y = "beaufort"),
      list(plot = scatter_plot, x = "year_numeric", y = "beaufort", xlim = c(1800, 1900)),
      list(plot = histogram, x = "wind_dir_math_real")
    ),
    ncols = 2
  )
)
