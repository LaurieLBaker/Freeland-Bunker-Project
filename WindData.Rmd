...
upset_df <- journal_wind_df %>%
  mutate(
    WindDescription = wind_mentioned == "Yes"&
  ...
  select(
    WindDescription, HasDirection, Winter,
    beaufort, wind_meteor, month_numeric, wind_speed_knots, year_numeric, vector_magnitude
  )
...
# Make sure columns that populate as integers are numeric 

```{r}

# right now only the month_numeric column does this but if you change the columns to include in the upset_df you may need to write additional lines for those if they populate as integers 
...
```{r}
sets <- c("WindDescription", "HasDirection", "Winter")
...
jpeg("UpSetWinter.jpg", width = 10, height = 10, units = "in", res = 300)
...
upset(
  ...
  attribute.plots = list(
    gridrows = 60,
    plots = list(
      list(plot = scatter_plot, x = "year_numeric", y = "beaufort", xlim = c(1500,1900)),
      list(plot = scatter_plot, x = "month_numeric", y = "beaufort"),
      list(plot = scatter_plot, x = "beaufort", y = "vector_magnitude"),
      list(plot = scatter_plot, x = "wind_meteor", y = "vector_magnitude")
    ),
    ncols = 2
  )
)
