...
read_letters <- read_letters %>%
<<<<<<< HEAD
  mutate(from = trimws(letter_from, "both")) %>%
=======
...
<<<<<<< HEAD
   mutate(year = year(date_mdy)) %>%
    count(from, year, sort = TRUE))

read_unique <- unique(read_count)

View(read_unique)

write_csv(read_count, file = "../data/received_letter_list.csv")
=======
...
<<<<<<< HEAD
write_csv(write_count, file = "../data/wrote_letter_list.csv")
...
ben_kitt <-letters %>%
<<<<<<< HEAD
  filter(grepl('Charles C. Burrill|C.C. Burrill|C C Buril', letter_from))
=======
...
bk_mention <-journals %>%
<<<<<<< HEAD
  filter(grepl('work in shop', journal_entry)) %>%
=======
...
# (Fix issues)
# journals %>%
#   filter(str_detect(journal_entry, pattern = 'Charles C. Burrill|C. C. Burrill'))
...
tb_mention <-journals %>%
<<<<<<< HEAD
  filter(grepl('lumber', journal_entry)) 
  filter(letter == "no letter")
```
